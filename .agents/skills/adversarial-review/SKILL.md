...
3. **Request the review** (headless; kimi takes six to eight minutes on a real
   ...
   ```
   scripts/request-review.sh plan review/<subject>/plan.md --reviewer=<kimi|gemini|claude>
   ```
...
2. Request the review:
   ```
   scripts/request-review.sh code review/<subject>/draft.diff --reviewer=<kimi|gemini|claude>
   ```
